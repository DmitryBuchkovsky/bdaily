            ...
            >
              <div className="mb-3 flex items-center justify-between">
                <button
                  onClick={() =>
                    onChange(
                      updateAt(items, i, {
                        priority: item.priority === "PRIMARY" ? "SECONDARY" : "PRIMARY",
                      }),
                    )
                  }
                  className={cn(
              ...
              <div className="space-y-3">
                <TicketAutocomplete
                  ...
                  onChange={(t) =>
                    onChange(
                      updateAt(items, i, { ticketId: t?.id, title: t?.summary ?? item.title }),
                    )
                  }
                />
                <FormInput
                  value={item.title}
                  onChange={(v) => onChange(updateAt(items, i, { title: v }))}
                  placeholder="Task title"
                />
                <RichTextEditor
                  value={item.goal ?? ""}
                  onChange={(v) => onChange(updateAt(items, i, { goal: v }))}
                ...
                <RichTextEditor
                  value={item.approach ?? ""}
                  onChange={(v) => onChange(updateAt(items, i, { approach: v }))}
                ...
                <div>
                  ...
                  <FormInput
                    type="date"
                    value={item.etaToDev ?? ""}
                    onChange={(v) => onChange(updateAt(items, i, { etaToDev: v || undefined }))}
                  />
                </div>
              </div>
            </div>
